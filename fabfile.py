...
if not hasattr(env, 'server'):
    ...
    env.project_name = u'{{project_name}}'
    env.shell = u'/bin/zsh -c'
    env.code_dir = u'~/sites/{{project_name}}'
    env.git_repo = u'git@github.com:lucassimon/{{project_repo}}git'
    env.settings = u'--settings={{project_name}}.settings.production'
    env.virtualenv = '~/venvs/{{project_repo}}/'
